...
### 🧩 死活题训练（26 题 · 4 阶梯）
吃子入门（8 题）→ 战术进阶（双打吃/一子双提/反提/追杀向边/对杀紧气，8 题）→ 死活手筋（点杀直三/曲三、倒扑、做眼、断吃，7 题）→ **段位冲刺**（征子十五手全歼、劫争全流程、轻灵转身，3 题）。
多手题带脚本化最强应手，支持重来与看答案演示。所有题解经自动化脚本验证（`npm run verify`，65 项检查）。

**判题方式：判结果，不判着手**（文档第六章）——提子/气数/眼形都是规则级硬事实：任何能达成题目目标的合法着法（含等价正解、不同顺序的紧气）都会被判对；多手题目标提前达成即成功。全部题解经自动化脚本验证（65 项）+ 可用 `cargo run --release --bin audit-puzzles` 做 KataGo ownership 独立审计（报告输出 docs/puzzle-audit.json；实测提示：开放局面下小目标的全局 ownership 受收官影响，仅适合封闭形死活题审计）。
