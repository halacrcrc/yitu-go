...
### 📖 新手教程（8 章闯关）
初识围棋 → 气 → 提子与救子 → 禁入点 → 打劫 → 死活基础 → 终局与胜负 → 布局常识。
全部为**互动式任务**：在棋盘上亲手完成提子、逃跑、做眼、点杀、寻劫材等操作，答对方可进入下一步，进度自动保存。

### 🧩 死活题训练（17 题 · 3 阶梯）
吃子入门（6 题）→ 战术进阶（双打吃/一子双提/反提/追杀向边，6 题）→ 死活手筋（点杀直三/曲三、倒扑、做眼、对杀紧气，5 题）。
多手题带脚本化最强应手，支持重来与看答案演示。所有题解经自动化脚本验证（`npm run verify`）。
